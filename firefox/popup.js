...
const saveRules = async (rules) => {
  ...
  api.runtime.sendMessage({ type: 'rulesUpdated' });
};

const readShortcut = async () => {
  const stored = await api.storage.sync.get(SHORTCUT_KEY);
...
const saveShortcut = async (shortcut) => {
  ...
  await api.storage.sync.set({ [SHORTCUT_KEY]: normalized });
};
...
const saveDefaultInterval = async (value) => {
  ...
  await api.storage.sync.set({ [DEFAULT_INTERVAL_KEY]: minutes });
};

const readPanelSize = async () => {
  const stored = await api.storage.sync.get(PANEL_SIZE_KEY);
...
const savePanelSize = async (size) => {
  await api.storage.sync.set({ [PANEL_SIZE_KEY]: size });
};
...
const renderTimers = async () => {
  ...
  const res = await api.runtime.sendMessage({ type: 'getTimers' });
  if (!res?.ok) return;
