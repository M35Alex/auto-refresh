...
const MIN_PERIOD_MINUTES = 0.1; // Chrome requires >=0.1 min in extension context.
...
const refreshTab = async (tabId) => {
  try {
    ...
    // Ignore tabs that cannot be refreshed (e.g., chrome://)
  }
};
...
const ensureTickAlarm = () => {
  chrome.alarms.create(TICK_ALARM, { periodInMinutes: Math.max(MIN_PERIOD_MINUTES, 0.1) });
};
